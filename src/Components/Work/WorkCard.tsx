...
const WorkCard = (props:WorkCardProps) =>{
    ...
    return(
        <span onClick={onClick} className={`w-full  rounded-3xl p-10 rounded-lg overflow-hidden ${props.class}`+ (props.loading? ' animate-pulse': ``) + (props.url !==""?` cursor-pointer bg-${props.color}-400 text-white transform transition duration-500 hover:scale-105`:` border border-1 border-${props.color}-400 text-${props.color}-400`)} >
