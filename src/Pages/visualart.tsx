...
const GraphicPage = () => {
    ...
    useEffect(() => {
        ...
        if (((content.length > 0)) && !isLoading) {
                ...
                switch (p.type) {
                    case GraphicsType.Illustration:
                        illustrationRenderer.push(
                            <div className="p-4  my-6  ">
                                <img className="w-full bg-blue-100 h-full min-h-96  rounded-lg" src={p.url} alt="illustration" />
                            </div>
                        );
                        break;
                    case GraphicsType.photograph:
                        photographRenderer.push(
                            <div className=" p-4 my-6 ">
                                <img className="w-full bg-blue-100  min-h-96  rounded-lg max-h-screen " src={p.url} alt="photograph" />
                            </div>
                        );
                        break;
                    case GraphicsType.gencode:
                        gencodeRenderer.push(
                            <div className=" p-4 my-6 ">
                                <img className="w-full bg-blue-100  min-h-96  rounded-lg max-h-screen " src={p.url} alt="gencode" />
                            </div>
                        );
            ...
            illustrationRenderer.push(
                <div className="w-full bg-blue-100 rounded-lg h-96 animate-pulse p-4  my-6  ">
        ...
        <div className="w-full min-h-screen pb-10">
            ...
            <div className="w-full pt-5 sm:p-10 text-center">
                {illustrationRenderer.length > 0 && <>
                    <span className="text-4xl font-bold text-blue-400">Illustrations</span>
                ...
                {gencodeRenderer.length > 0 && <>
                    <span className={" text-center  text-4xl font-bold text-blue-400" + (illustrationRenderer.length > 0 ? " mt-10" : "")}>Generative Coding
                ...
                {photographRenderer.length > 0 && <>
                    <span className={"text-4xl font-bold text-blue-400" + (illustrationRenderer.length > 0 || gencodeRenderer.length > 0 ? " mt-10" : "")}>Photography</span>
                    <div className="flex flex-col sm:block sm:masonry before:box-inherit after:box-inherit">
                        {photographRenderer}
                    </div>
                </>
